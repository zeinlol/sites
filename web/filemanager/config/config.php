...
if ($_SESSION['FileUploadID']) {

    $LastPath = __DIR__ . '/../../' . 'source' . '/';
    $NewDir = realpath($LastPath) . DIRECTORY_SEPARATOR . $_SESSION['__id'];

    if (!is_dir($NewDir)) {
        @mkdir($NewDir);
    }

    $LastPath = __DIR__ . '/../../' . 'thumbs' . '/';
    $NewDir = realpath($LastPath) . DIRECTORY_SEPARATOR . $_SESSION['__id'];
    ...
    $config['upload_dir'] = $config['upload_dir'] . $_SESSION['__id'] . '/';
    $config['current_path'] = $config['current_path'] . $_SESSION['__id'] . '/';
    $config['thumbs_base_path'] = $config['thumbs_base_path'] . $_SESSION['__id'] . '/';
}
