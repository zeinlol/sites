				...
				foreach($disp_typeArray as $disp_type){
					...
					//-- Inputs--//
						...
						foreach ($options_ar as $index=>$option){
							...
							else $value=!empty($this->selected_flt[$key])?$this->selected_flt[$key]:'';
							...
							$innerHTML.='<input name="'.$option->name.'" value="'.$value.'" type="search" placeholder="'.$placeholder.'" size="'.$size.'" maxlength="'.$maxlength.'" id="'.$display_key.'_'.$index.'" '.$script.'>'.$symbol_end;
