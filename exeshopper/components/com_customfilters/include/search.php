...
class CfSearchHelper{
	 ...
	 * @param   boolean  $matching  Flag to indicate the type of matching [optional]
	 *
	...
	public function tokenize($input, $lang, $matching = 'any')
	{
		...
		$searchfields=$componentParams->get('keyword_searchfield',array('l.product_name','l.product_s_desc','catlang.category_name','mflang.mf_name','custom'),'array');
		...
		$foundInDb=array();
		if(in_array('catlang.category_name', $searchfields))$foundInDb['category']=self::getCategories($primary_tokens);
		if(in_array('mflang.mf_name', $searchfields))$foundInDb['manufacturer']=self::getManufacturers($primary_tokens);
		if(in_array('custom', $searchfields))$foundInDb['customvalue']=self::getCustomfieldValues($primary_tokens);
		...
		if($matching=='any' && count($primary_tokens)<4){
			foreach ($primary_tokens as $i=>$ptoken){
				...
				//no identical
				$phrases[]=$ptoken->term.$spacer.implode($spacer, $phrase_parts);
				//1st word identical
				if(!empty($ptoken->identical))$phrases[]=$ptoken->identical->term.$spacer.implode($spacer, $phrase_parts);
				//rest identical
				if(!empty($identical_phrase_parts))$phrases[]=$ptoken->term.$spacer.implode($spacer, $identical_phrase_parts);


				//in case the phrase has more than 2 terms, create combinations reversing it
				if($n>=2){
					//no identical
					$phrases[]=$ptoken->term.$spacer.implode($spacer, array_reverse($phrase_parts));
					//1st word identical
					if(!empty($ptoken->identical))$phrases[]=$ptoken->identical->term.$spacer.implode($spacer, array_reverse($phrase_parts));
					//rest identical
					if(!empty($identical_phrase_parts))$phrases[]=$ptoken->term.$spacer.implode($spacer, array_reverse($identical_phrase_parts));
				}
			}
		}
	...
	public function formatTokens($inputs,$foundInDb)
	{
		...
		if($db_findings_counter>0)$max_level=count($foundInDb)-1;
		else $max_level=0;
		
		foreach ($inputs as $input){
			if(is_object($input))$token=$input;
			//create the object
			else{
				$token=new stdClass();
				$token->term=trim($input);
				$token->phrase=true;
			...
			foreach ($foundInDb as $key=>$array){
				...
				if(empty($array) || !empty($token->$key))continue;
		...
		//recursion
		if($level<=$max_level)$this->formatTokens($tokens, $foundInDb);
		return $tokens;

	}

	/**
	 * Gets a token and create new substracting words (e.g. category, manufacturer)
	 *
	 * @param 	object 	$phrase
	 ...
	 * @since	2.2.0
	 */
	public function subtractFromPhrase($token,$dbrecords,$key='category')
	{ 
		$new_tokens=array();
		...
		foreach ($dbrecords as $id=>$dbrecord){
			...
			if(strpos($token->term, $dbrecord)!==false){ 
				if($canbeplurar==false)$new_term=str_replace($dbrecord, '', $token->term);
				else $new_term=preg_replace('/'.$dbrecord.'([^\x30-\x7F]|[a-zA-Z]){0,2}\s/', '', $token->term);
	...
	public function getCategories($tokens)
	{
		...
		if(!is_array($tokens)){
			...
			foreach ($tokens as $token){
				...
				if(isset($token->identical))$whereOr[]='cl.category_name LIKE '.$db->quote($db->escape($token->identical->term, true).'%', false);
			}
	...
	public function getManufacturers($tokens)
	{
		...
		if(!is_array($tokens)){
			...
			foreach ($tokens as $token){
				...
				if(isset($token->identical))$whereOr[]=('ml.mf_name LIKE '.$db->quote($db->escape($token->identical->term, true).'%', false));
			}
			$query->where(implode(' OR ', $whereOr));
		}
	 ...
	 * @param 	array $array
	 * @param 	string $key  an object property
	 * @param 	string $value an object property
	 * @since	2.2.0
	 */
