...
if ($this->allowRating || $this->allowReview || $this->showRating || $this->showReview) {
	...
	for( $num = 0; $num<=$maxrating; $num++ ) {
		$stars[] = '
				<span title="'.(vmText::_( "COM_VIRTUEMART_RATING_TITLE" ).$num.'/'.$maxrating).'" class="vmicon ratingbox" style="display:inline-block;width:'. 20*$maxrating.'px;">
	...
	if ($review_editable) {
		...
		<form method="post"
			...
			<?php if($this->allowRating and $review_editable) { ?>
				...
				<h3><?php echo vmText::_( 'COM_VIRTUEMART_WRITE_REVIEW' );
		...
		<span><?php echo vmText::_( 'COM_VIRTUEMART_REVIEW_COUNT' ) ?>
			<input type="text" value="0" size="4" name="counter" maxlength="4" readonly="readonly"/>
				...
				if($review_editable and $this->allowReview) {
					?>
					<br/>
					<input class="highlight-button vina-btn" type="submit" onclick="return( check_reviewform());"
				...
				<?php } else if($review_editable and $this->allowRating) { ?>
					<input class="highlight-button vina-btn" type="submit" name="submit_review"
			...
			foreach ($this->rating_reviews as $review) {
				...
				if (!empty($this->rating_reviews) && $review->published) {
					...
					<div class="<?php echo $color ?>">
						...
						<div class="auhor">
							<span class="header-author"><?php echo JTEXT::_('VM_LANG_POST_AUTHOR'); ?>: </span>
