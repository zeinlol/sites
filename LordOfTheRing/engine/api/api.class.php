...
if(!class_exists('DLE_API'))
{
	class DLE_API
	{
        ...
        function change_user_fullname ($user_id, $new_fullname)
		{
			$user_id = intval( $user_id );
			$new_fullname = $this->db->safesql( $new_fullname );
			$count_arr = $this->load_table(USERPREFIX."_users", "count(user_id) as count", "fullname = '$new_fullname'");
			$count = $count_arr['count'];
			
			if( $count > 0 ) return false;

			$old_fullname_arr = $this->load_table(USERPREFIX."_users", "fullname", "user_id = '$user_id'");
			$old_fullname = $old_fullname_arr['fullname'];
			$this->db->query( "UPDATE " . PREFIX . "_post SET autor='$new_fullname' WHERE autor='{$old_fullname}'" );
			$this->db->query( "UPDATE " . PREFIX . "_comments SET autor='$new_fullname' WHERE autor='{$old_fullname}' AND is_register='1'" );
			$this->db->query( "UPDATE " . USERPREFIX . "_pm SET user_from='$new_fullname' WHERE user_from='{$old_fullname}'" );
			$this->db->query( "UPDATE " . PREFIX . "_vote_result SET fullname='$new_fullname' WHERE fullname='{$old_fullname}'" );
			$this->db->query( "UPDATE " . PREFIX . "_images SET author='$new_fullname' WHERE author='{$old_fullname}'" );
			$this->db->query( "update " . USERPREFIX . "_users set fullname = '$new_fullname' where user_id = '$user_id'" );
			return true;
