									...
									<?php } else if ($action['type'] == 'menu') { ?>
										<div class="btn-group"  data-toggle="tooltip" data-container="body" title="<?php echo $action['title']; ?>">
											...
											<ul class="dropdown-menu pull-right">
												...
												<li><a href="index.php?route=customer/customer/login&token=<?php echo $token; ?>&customer_id=<?php echo $customer['customer_id']; ?>&store_id=<?php echo $store_id; ?>" target="_blank"><?php echo $store['name']; ?></a></li>
												<?php } ?>
											</ul>
										</div>
									<?php } else { ?>
...
bull5i.filter=function(){url='index.php?route=customer/customer&token=<?php echo $token; ?>&sort=<?php echo $sort; ?>&order=<?php echo $order; ?>';
...
$('input[name=\'filter_<?php echo $column; ?>\']').autocomplete({source:function(request,response){$.ajax({url:'index.php?route=customer/customer/autocomplete2&token=<?php echo $token; ?>&filter_<?php echo $column; ?>='+encodeURIComponent(request),dataType:'json',success:function(json){response($.unique($.map(json,function(item){
