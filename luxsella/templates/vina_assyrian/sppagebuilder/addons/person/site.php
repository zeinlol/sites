...
function sp_person_addon($atts) {
	...
	if($facebook || $twitter || $google_plus || $youtube || $linkedin || $pinterest || $flickr || $dribbble || $behance || $instagram) {
		...
		if($email) 			$social_icons .= '<li><a target="_blank" href="' . $email . '"><i class="fa fa-envelope"></i></a></li>';
	...
	if($image) {
		...
		if($name || $designation) {
			$output  .= '<div class="person-info"><div class="sppb-person-information">';
			if($name) $output  .= '<h4 class="sppb-person-name">' . $name . '</h4>';
